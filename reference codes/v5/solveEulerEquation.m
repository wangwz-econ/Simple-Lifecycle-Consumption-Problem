...
for ixA = 1:1:numPtsA                               % points on asset grid
    ...
    for ixY = 1:1:numPtsY                                     % for each point on the income grid (last income)
        EV(T,ixA,ixY)  = incTransitionMrx(:,ixY)'*realisedV;  % continuation value (EV_T seen from T-1)
        EdU(T,ixA,ixY) = incTransitionMrx(:,ixY)'*realiseddU; % expected marginal utility (EdU_T seen from T-1)
    end %ixY
...
for ixt=(T-1):-1:1                              % Loop from time T-1 to 1
    ...
    for ixA = 1:1:numPtsA                       % Loop through points on the asset grid
        ...
        for ixY = 1:1:numPtsY                                                  % for each point on the income grid (last income)
            EV(ixt, ixA, ixY)  = incTransitionMrx(ixY,:)*realisedvalues;       % continuation value (EV_t seen from t-1)
            EdU(ixt, ixA, ixY) = incTransitionMrx(ixY,:)*realisedMargUtility;  % expected marginal utility (EdU_t seen from t-1)
        end %ixY
